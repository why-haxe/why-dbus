...
class Property<T> implements ReadWriteProperty<T> {
	...
	public function get():Promise<T> {
		final value = properties.get(iface, name)
			.next(v -> v.signature == signature ? Promise.resolve((v.value:T)) : new Error('Unexpected return type for member "$name" on interface "$iface". Expected "$signature" but got "${v.signature}"'));
